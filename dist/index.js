...
app.get('/api/notify', async (req, res) => {
    ...
    `);
        ...
        for (const u of rows) {
            ...
            const parts = [];
            if (canClaim)
                parts.push('⏰ Free chips ready to claim!');
            if (canSpin)
                parts.push('🎰 Daily spin wheel available!');
            await tgSend(u.tg_id, `👋 Hey ${u.first_name || 'Player'}!\n\n${parts.join('\n')}\n\nYou have *${u.chips.toLocaleString()} chips* waiting.`, { reply_markup: PLAY_BTN });
            sent++;
