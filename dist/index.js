    ...
    try {
       ...
       RETURNING *`, [tgId, tgUser.username || null, tgUser.first_name || null, tgUser.photo_url || null,
        ...
        if (lastLogin !== today) {
            ...
            const { rows: updated } = await db.query(`UPDATE pf_users SET chips = chips + $1, streak_days = $2, last_login_date = $3
         WHERE tg_id = $4 RETURNING *`, [bonus, newStreak, today, tgId]);
            await (0, db_1.logTransaction)(tgId, 'streak', bonus, `Day ${newStreak} login bonus`);
