...
import { registerForSF, cancelSFRegistration, getSFStatus, getSFAdminStats, SF_CONFIGS, SFRoomId } from './spinFlip'
    ...
    // Admin commands (only for admin tgId)
      ...
      if (text?.startsWith('/admin credit ')) {
        ...
        for (const reg of regs) {
          const cfg = SF_CONFIGS[reg.room_id as SFRoomId]
          if (!cfg) continue
          await db.query('UPDATE pf_users SET chips = chips + $1 WHERE tg_id=$2', [cfg.buyIn, reg.tg_id])
          await logTransaction(reg.tg_id, 'sf_cancel', cfg.buyIn, `Admin SF cleanup refund: ${cfg.name}`)
          refunded++
        }
        await db.query(`
